...
function renderWorld(oncomplete){
    ...
    let cb1 = dropUnit(ground_bricks[14], GRAPHICS.question_block, null, -84);
    // coinboxes.push(cb1);
    // hitables.push(cb1);
    // collidables.push(cb1);

    let b4 = dropUnit(ground_bricks[9], GRAPHICS.ground_pipe);
    let b5 = dropUnit(ground_bricks[8], GRAPHICS.question_block, -30, -200);
	...
	// }
    for (let i = 0; i<3; i++){
        bb1 = dropUnit(ground_bricks[7], GRAPHICS.block_brick, i * 2000, -64);
		hitables.push(bb1);
		collidables.push(bb1);
	}
